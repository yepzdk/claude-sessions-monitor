...
- The web dashboard's live list groups a project once it has more than one live session, so a busy machine reads by project instead of as a wall of cards. A project with no header of its own names itself instead, on its lone session's card and on the row that folds its stopped sessions away. A Needs Input session in a collapsed group lifts to the top of the list instead of staying hidden until the group reopens
